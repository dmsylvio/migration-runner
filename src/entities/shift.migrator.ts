...
// ✅ Novo schema (Drizzle):
// shift: id(text uuidv7), old_id(text), name(text not null), created_at, updated_at
// ✅ UPSERT por old_id ou name
const TARGET_TABLE = "shift";

interface LegacyShiftRow extends RowDataPacket {
  id: number; // int(11) PK
  turno: string; // varchar(255) -> name
  created_at: any; // timestamp
}

async function upsertShift(row: LegacyShiftRow) {
  const createdAt = row.created_at ? new Date(row.created_at) : new Date();
  const updatedAt = createdAt;
  const oldId = String(row.id);
  const name = row.turno?.trim() || "";
...
async function seed(runId: number) {
  ...
  while (true) {
    const [rows] = await mariadb.query<LegacyShiftRow[]>(
      `
      SELECT id, turno, created_at
      FROM tb_turno
      WHERE id > :lastPk
      ORDER BY id
      LIMIT :limit
...
async function sync(runId: number) {
  ...
  // Como tb_turno não tem updated_at, vamos usar uma estratégia diferente:
...
export const shiftMigrator: Migrator = {
  entity: ENTITY,
  seed,
  sync,
};
