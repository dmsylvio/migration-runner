...
const ENTITY = "company_representative";

// ✅ Novo schema (Drizzle):
// company_representative: id(text uuidv7), old_id(text), company_id(text not null), full_name, cpf_number,
// rg_number, issuing_authority, phone, whatsapp, position, created_at, updated_at
// ✅ UPSERT por old_id
const TARGET_TABLE = "company_representative";

interface LegacyCompanyRepresentativeRow extends RowDataPacket {
  id: number; // int(11) PK
...
async function upsertCompanyRepresentative(
  row: LegacyCompanyRepresentativeRow,
) {
  ...
  if (!companyId) {
    throw new Error(
      `Company not found for legacy UUID: ${row.empresa_id} (representative: ${oldId})`,
    );
  }
...
async function seed(runId: number) {
  ...
  while (true) {
    const [rows] = await mariadb.query<LegacyCompanyRepresentativeRow[]>(
      `
    ...
    try {
      for (const r of rows) {
        await upsertCompanyRepresentative(r);
      }
    } catch (e: any) {
      await logError({
        runId,
...
export const companyRepresentativeMigrator: Migrator = {
  entity: ENTITY,
  seed,
  sync,
};
