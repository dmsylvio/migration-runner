...
const ENTITY = "institution_supervisor";

// ✅ Novo schema (Drizzle):
// institution_supervisor: id(text uuidv7), old_id(text), institution_id(text not null), full_name, cpf_number,
// rg_number, issuing_authority, phone, whatsapp, position, created_at, updated_at
// ✅ UPSERT por old_id
const TARGET_TABLE = "institution_supervisor";

interface LegacyInstitutionSupervisorRow extends RowDataPacket {
  id: number; // int(11) PK
...
async function upsertInstitutionSupervisor(
  row: LegacyInstitutionSupervisorRow,
) {
  ...
  if (!institutionId) {
    throw new Error(
      `Institution not found for legacy UUID: ${row.instituicao_id} (supervisor: ${oldId})`,
    );
  }
...
async function seed(runId: number) {
  ...
  while (true) {
    ...
    try {
      for (const r of rows) {
        await upsertInstitutionSupervisor(r);
      }
    } catch (e: any) {
      await logError({
        runId,
...
export const institutionSupervisorMigrator: Migrator = {
  entity: ENTITY,
  seed,
  sync,
};
