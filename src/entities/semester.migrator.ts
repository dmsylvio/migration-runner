...
const ENTITY = "semester";

// ✅ Novo schema (Drizzle):
// semester: id(text uuidv7), old_id(text), name(text not null), created_at, updated_at
// ✅ UPSERT por old_id ou name
const TARGET_TABLE = "semester";

interface LegacySemesterRow extends RowDataPacket {
  id: number; // int(11) PK
  semestre: string; // varchar(30) -> name
}

async function upsertSemester(row: LegacySemesterRow) {
  const now = new Date();
  const oldId = String(row.id);
  const name = row.semestre?.trim() || "";
...
async function seed(runId: number) {
  ...
  while (true) {
    const [rows] = await mariadb.query<LegacySemesterRow[]>(
      `
      SELECT id, semestre
      FROM tb_semestre
      WHERE id > :lastPk
...
async function sync(runId: number) {
  ...
  // Como tb_semestre não tem updated_at, vamos usar uma estratégia diferente:
...
export const semesterMigrator: Migrator = {
  entity: ENTITY,
  seed,
  sync,
};
