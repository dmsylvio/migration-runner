...
const ENTITY = "marital_status";

// ✅ Novo schema (Drizzle):
// marital_status: id(text uuidv7), old_id(text), name(text not null), created_at, updated_at
// ✅ UPSERT por old_id ou name
const TARGET_TABLE = "marital_status";

interface LegacyMaritalStatusRow extends RowDataPacket {
  id: number; // int(11) PK
  estado_civil: string; // varchar(255) -> name
  created_at: any; // datetime
}

async function upsertMaritalStatus(row: LegacyMaritalStatusRow) {
  const createdAt = row.created_at ? new Date(row.created_at) : new Date();
  const updatedAt = createdAt;
  const oldId = String(row.id);
  const name = row.estado_civil?.trim() || "";
...
async function seed(runId: number) {
  ...
  while (true) {
    const [rows] = await mariadb.query<LegacyMaritalStatusRow[]>(
      `
      SELECT id, estado_civil, created_at
...
async function sync(runId: number) {
  ...
  // Como tb_estado_civil não tem updated_at, vamos usar uma estratégia diferente:
...
export const maritalStatusMigrator: Migrator = {
  entity: ENTITY,
  seed,
  sync,
};
