...
const ENTITY = "educational_institution";

// ✅ Novo schema (Drizzle):
// educational_institution: id(text uuidv7), old_id(text), name(text not null), created_at, updated_at
// ✅ UPSERT por old_id ou name
const TARGET_TABLE = "educational_institution";

interface LegacyEducationalInstitutionRow extends RowDataPacket {
  co_seq_confinstituicao: number; // int(10) PK
  no_instituicao: string; // text -> name
  dt_publicado: any; // date -> created_at
}

async function upsertEducationalInstitution(
  row: LegacyEducationalInstitutionRow,
) {
  const createdAt = row.dt_publicado ? new Date(row.dt_publicado) : new Date();
  const updatedAt = createdAt;
  const oldId = String(row.co_seq_confinstituicao);
  const name = row.no_instituicao?.trim() || "institution_missing_name";
...
async function seed(runId: number) {
  ...
  while (true) {
    const [rows] = await mariadb.query<LegacyEducationalInstitutionRow[]>(
      `
      SELECT co_seq_confinstituicao, no_instituicao, dt_publicado
      FROM tb_confinstituicao
      WHERE co_seq_confinstituicao > :lastPk
      ORDER BY co_seq_confinstituicao
      LIMIT :limit
      `,
    ...
    try {
      for (const r of rows) {
        await upsertEducationalInstitution(r);
      }
    } catch (e: any) {
      await logError({
        runId,
        entity: ENTITY,
        sourcePk: String(rows[0]?.co_seq_confinstituicao ?? ""),
    ...
    lastPk = rows[rows.length - 1].co_seq_confinstituicao;
...
async function sync(runId: number) {
  ...
  // Como tb_confinstituicao não tem updated_at, vamos usar uma estratégia diferente:
  // sincronizar apenas novos registros baseado no co_seq_confinstituicao
  let lastPk = 0;
...
export const educationalInstitutionMigrator: Migrator = {
  entity: ENTITY,
  seed,
  sync,
};
