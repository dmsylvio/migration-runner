...
const ENTITY = "education_level";

// ✅ Novo schema (Drizzle):
// education_level: id(text uuidv7), old_id(text), name(text not null), created_at, updated_at
// ✅ UPSERT por old_id ou name
const TARGET_TABLE = "education_level";

interface LegacyEducationLevelRow extends RowDataPacket {
  id: number; // int(11) PK
  nivel: string; // varchar(30) -> name
}

async function upsertEducationLevel(row: LegacyEducationLevelRow) {
  const now = new Date();
  const oldId = String(row.id);
  const name = row.nivel?.trim() || "";
...
async function seed(runId: number) {
  ...
  while (true) {
    const [rows] = await mariadb.query<LegacyEducationLevelRow[]>(
      `
      SELECT id, nivel
...
async function sync(runId: number) {
  ...
  // Como tb_escolaridade não tem updated_at, vamos usar uma estratégia diferente:
...
export const educationLevelMigrator: Migrator = {
  entity: ENTITY,
  seed,
  sync,
};
