...
// ✅ Novo schema (Drizzle):
// gender: id(text uuidv7), old_id(text), name(text not null), created_at, updated_at
// ✅ UPSERT por old_id ou name
const TARGET_TABLE = "gender";

interface LegacyGenderRow extends RowDataPacket {
  id: number; // int(11) PK
  sexo: string; // varchar(255) -> name
  created_at: any; // datetime
}

async function upsertGender(row: LegacyGenderRow) {
  const createdAt = row.created_at ? new Date(row.created_at) : new Date();
  const updatedAt = createdAt;
  const oldId = String(row.id);
  const name = row.sexo.trim();
...
async function seed(runId: number) {
  ...
  while (true) {
    const [rows] = await mariadb.query<LegacyGenderRow[]>(
      `
...
async function sync(runId: number) {
  ...
  // Como tb_sexo não tem updated_at, vamos usar uma estratégia diferente:
...
export const genderMigrator: Migrator = {
  entity: ENTITY,
  seed,
  sync,
};
